...
for zi=1:length(n2pad)
    ...
    % FFT and amplitude
    sigampl   = 2*abs( fft(signal,zeropadN) );  % the 2nd input is the TOTAL amount of timepoints, not the extra zero-padding points!
    
    % one of the two normalization steps
    %sigampl = sigampl / 1;
    %sigampl = sigampl / zeropadN;
    sigampl = sigampl / length(signal);
